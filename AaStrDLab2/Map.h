...
public:
	...
	{
	public:
		Node(bool color = RED, T key = T(), Node* parent = nullptr, Node* left = nullptr, Node* right = nullptr, T1 value = T1()) :color(color), key(key), parent(parent), left(left), right(right), value(value) {}
	...
	Map(Node* Top = nullptr, Node* Leaf = new Node(0)) :Top(Top), Leaf(Leaf) {}
...
private:
	...
	void ListPairs(Node* tree, List<haffman<T, T1>>* list) {

		if (tree != Leaf) {

			ListPairs(tree->left, list);

			haffman<T, T1> current(tree->key, tree->value);
	...
	List<haffman<T, T1>>* get_pairs() {

		List<haffman<T, T1>>* list = new List<haffman<T, T1>>();
